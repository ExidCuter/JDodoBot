...
public class KickUserFromGuild extends IFunction {
    public KickUserFromGuild(String command, String description, String usage) {
    ...
    public void trigger(MessageParams p_messageParams) {
        ...
        if(AdminUtils.isAdminOfGuild(p_messageParams.getUser(), p_messageParams.getGuild())) {
            ...
            if (members.size() > 0) {
                members.forEach(p_member -> p_messageParams.getGuild().getController().kick(p_member).queue());
                p_messageParams.getMessage().addReaction("\u2705").queue();
            } else
                p_messageParams.getTextChannel().sendMessage("Please mention users to kick").queue();
        } else
